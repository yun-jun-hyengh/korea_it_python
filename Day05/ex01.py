...
형식)
for 변수 in range(start(시작값), stop(끝값), step(증감)):
    반복할 문장

range() 함수
- 파이썬에서 range() 함수는 일정 범위의 연속된 정수를 생성하는데 사용된다 
주로 for문과 함께 사용되어 일정 횟수만큼 반복을 수행한다.

사용방법
range(stop) : 0 부터 stop -1 까지의 숫자를 생성한다 
range(start, stop) :  start부터 stop -1까지의 숫자를 생성한다 
range(start, stop, step) : start부터 stop -1까지 step 간격으로 숫자를 생성한다 
'''
